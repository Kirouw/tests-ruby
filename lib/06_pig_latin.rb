def translate(string)
	...
	string_array.each do |w|
		...
		two_caract = w[0..1] #Selection des 2 premiers caractère ("Hello" => "He")
		three_caract = w[0..2] #Selection des 3 premiers caractère ("Hello" => "Hel")
		...
		if vowel.include?(one_caract) #Si le premier caractère contient une voyelle du tableau vowel
			...
			if three_consonant.include?(three_caract) #Si les 3 premiers caractères contiennent 3 consonnes inclus dans le tableau three_consonant
				...
				final_string << w+three_first+"ay" #On ajout le mot restant + les 3 premiers caractères découpés + "ay" à la fin et on ajoute au tableau final_string
				...
				final_string << w+two_first+"ay" #On ajout le mot restant + les 2 premiers caractères découpés + "ay" à la fin et on ajoute au tableau final_string
			else #Sinon
				first = w.slice!(0) #On decoupe le mot pour selectionner le premier caractère
				final_string << w+first+"ay" #On ajout le mot restant + le premier caractère découpés + ay et on ajoute au tableau final_string
			end
		end
	end
