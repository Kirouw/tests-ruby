def translate(string)
	...
	string_array = string.split(' ') #Transformation du string en tableau séparé par des espaces

	final_string = [] #Création d'un tableau vide pour entrer les résultats

	string_array.each do |w|

		one_caract = w[0] #Selection du premier caractère
		two_caract = w[0]+w[1] #Selection des 2 premiers caractère
		three_caract = w[0]+w[1]+w[2] #Selection des 3 premiers caractère
		...
		if vowel.include?(one_caract) #Si le premier caractère contient une voyelle du tableau vowel
			...
			if three_consonant.include?(three_caract) #Si les 3 premiers caractères contiennent 3 consonnes inclus dans le tableau three_consonant
				...
				final_string << w+three_first+"ay" #On ajout le mot + les 3 premiers caractères + "ay" à la fin et on ajout au tableau final_string
				...
				final_string << w+two_first+"ay" #On ajout le mot + les 2 premiers caractères + "ay" à la fin et on ajout au tableau final_string
			else #Sinon
				first = w.slice!(0) #On decoupe le mot pour selectionner le premier caractère
				final_string << w+first+"ay" #On ajout le mot + le premier caractère + ay et on ajout au tableau final_string
			end
		end
	end
	return final_string.join(" ") #On transforme le tableau en string séparer par des espaces
end
